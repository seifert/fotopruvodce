...
def validate_thumbnail(value):
    if not value.instance.id and (
        value.size > settings.THUMB_MAX_UPLOAD_SIZE
        or value.width > settings.THUMB_MAX_SIZE[0]
        or value.height > settings.THUMB_MAX_SIZE[1]
    ):
...
def validate_photo(value):
    if not value.instance.id and (
        value.size > settings.PHOTO_MAX_UPLOAD_SIZE
        or value.width > settings.PHOTO_MAX_SIZE[0]
        or value.height > settings.PHOTO_MAX_SIZE[1]
    ):
...
class Photo(models.Model):
    ...
    title = models.CharField(verbose_name="Název fotky", max_length=128)
    ...
    deleted = models.BooleanField(verbose_name="Smazáno", default=False, db_index=True)
    ...
    thumbnail_height = models.PositiveIntegerField(verbose_name="Výška náhledu")
    thumbnail_width = models.PositiveIntegerField(verbose_name="Šířka náhledu")
    thumbnail = models.ImageField(
        verbose_name="Náhled",
        upload_to=upload_thumb_fullpath,
        height_field="thumbnail_height",
        width_field="thumbnail_width",
        validators=[validate_thumbnail],
    )
    photo_height = models.PositiveIntegerField(verbose_name="Výška fotky")
    photo_width = models.PositiveIntegerField(verbose_name="Šířka fotky")
    photo = models.ImageField(
        verbose_name="Fotka",
        upload_to=upload_photo_fullpath,
...
class SeriesPhoto(models.Model):
    ...
    image = models.ImageField(
        verbose_name="Fotka",
        upload_to=upload_series_photo_fullpath,
        height_field="height",
        width_field="width",
        validators=[validate_photo],
    )
    ...
    def __repr__(self):
        return "<{}: {} #{}>".format(self.__class__.__name__, self.photo.title, self.id)
