...
from fotopruvodce.photos.forms import Add as PhotoAddForm
from fotopruvodce.photos.forms import Edit as PhotoEditForm
from fotopruvodce.photos.forms import Evaluation as EvaluationForm
from fotopruvodce.photos.forms import SeriesPhotoInline as SeriesPhotoInlineForm
from fotopruvodce.photos.forms import WorkshopInline as WorkshopInlineForm
from fotopruvodce.photos.models import Comment, Photo, Rating, Section
...
def total_score_listing(request):
    ...
    query = (
        Rating.objects.values(
            ...
            "photo__timestamp",
            "photo__thumbnail",
            "photo__user__username",
        )
        .filter(photo__deleted=False, photo__active=True)
        .annotate(Avg("rating"), Count("rating"), Sum("rating"))
        .order_by("-rating__sum")
    )
...
def comments_listing(request, action, **kwargs):
    ...
    return render(request, COMMENTS_LISTING_ACTION_TO_TEMPLATE[action], context)
...
def months(request):
    months = Photo.objects.filter(deleted=False, active=True).dates(
...
def detail(request, obj_id):
    ...
    if request.method == "POST":
        ...
        form = EvaluationForm(
            request.POST, logged_user=request.user, photo=obj, logged_user_rating=rating
        )

        if form.is_valid():
            if form.cleaned_data["content"]:
                comment = Comment(
                    content=form.cleaned_data["content"], photo=obj, user=request.user
                )
            ...
            if form.cleaned_data["rating"] is not None:
                if rating is not None:
                    ...
                    rating = Rating(
                        rating=form.cleaned_data["rating"], photo=obj, user=request.user
                    )
...
def listing_account(request):
    ...
    query = (
        Photo.objects.select_related("section")
        .filter(
            user=request.user,
            deleted=False,
        )
...
def add(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            workshop_form = WorkshopInlineForm(request.POST, instance=created_photo)
            ...
            series_photo_form = SeriesPhotoInlineForm(request.POST, request.FILES)
...
def edit(request, photo_id):
    obj = get_object_or_404(Photo, id=photo_id, user=request.user, deleted=False)
