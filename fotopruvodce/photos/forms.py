...
class SeriesPhoto(forms.ModelForm):
    ...
    image = forms.ImageField(
        label="Další fotka do série",
        required=False,
        help_text="Maximální "
        "povolené rozměry fotky jsou {}×{}px a velikost souboru do {}.".format(
            settings.PHOTO_MAX_SIZE[0],
            settings.PHOTO_MAX_SIZE[1],
...
WorkshopInline = forms.inlineformset_factory(
    photos_models.Photo,
    workshop_models.Workshop.photos.through,
    fields=("workshop",),
    extra=1,
)
...
class Evaluation(forms.Form):
    ...
    RATINGS = tuple((j, j) for j in (str(i) for i in range(10, -1, -1))) + (
        ("", "Nebodovat"),
    )
